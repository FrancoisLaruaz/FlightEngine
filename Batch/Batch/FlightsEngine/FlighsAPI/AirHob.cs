...
{
    public static class AirHob
    {
        public static string Key = "f2f6973d-17ef-4";
        // Production :public static string Key = "d4c031b3-5873-4";


        // Each product API are set by default to 30 calls/second, and 1000 calls/day during development in the sandbox environment. Once approved for production, the daily calls limit is increased to 10,000. 
        public static bool SearchFlights(AirlineSearch filter)
        {
            bool result = false;
            try
            {
                Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " ***  START AirHob ***");
                MakeRequest(filter);

                Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " ***  END AirHob ***");
            }
            catch (Exception e)
            {
        ...
        static  bool MakeRequest(AirlineSearch filters)
        {
            ...
            try
            {
                string url = "https://dev-sandbox-api.airhob.com/sandboxapi/flights/v1.3/search";
